...
replace_once(
    ...
    "debug pane positioning hook",
)

marker = '''    function Alphabetically(A, B) {
...
block = r'''    function Alphabetically(A, B) {
        return String(A).localeCompare(String(B));
    }
...
replace_once(marker, block, "keyboard/debug function block")
...
if entry not in ctext:
    ...
    ctext = ctext.replace(heading, heading + entry, 1)
