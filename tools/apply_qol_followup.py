...
# The Preview-only beta names are safe to retire before release.
text = text.replace("BylineNextToDate", "CompactTitleLayout")
text = text.replace("YesterdayNextToDate", "HideYesterdayHelpRow")

# Put the adjustable blank space INSIDE NYT's list container instead
# of changing the CSS-grid gap between the TI and GB rows.
replace_n(
    ...
    2,
    "word-area variable -> list-container gap"
)
...
replace_n(
    ...
    2,
    "external row gap fixed to zero"
)

# Render top stat cards with the same new-word highlight semantics.
sub_once(
    r'''    function RenderMainStats\(Panel, Stats\) \{.*?\n    \}\n\n    function RenderHints''',
...
# Use exact word lengths instead of a 7+ bucket.
sub_once(
    r'''    function RenderWordsByLength\(Panel, PreviousOpenStates, Stats\) \{.*?\n    \}\n\n    function RenderFoundAndUnfoundWords''',
...
sub_once(
    r'''    function CalculateStats\(FoundDictionaryWords\) \{.*?\n    function GetLengthBucket\(Length\) \{.*?\n    \}\n''',
...
''',
    "CalculateStats/GetLengthBucket"
)

# Add a generic helper for stat cards that depend on any currently
# highlighted word matching a predicate.
marker = '''    function CreateSettingsSection(TitleText) {'''
helper = '''    function ApplyRecentWordGroupHighlight(Element, Predicate) {
        let LatestExpiry = 0;
        const Now = Date.now();
...
'''
replace_once(marker, helper + marker, "recent-word stat helper")

# AddStat now returns the card so callers can decorate it.
replace_once(
    '''        Card.append(ValueElement, LabelElement);
        Container.appendChild(Card);
    }

    function AddLengthStat''',
    '''        Card.append(ValueElement, LabelElement);
        Container.appendChild(Card);
        return Card;
    }
...
# Replace Settings with the requested grouping/order/copy.
sub_once(
    r'''    function CreateSettingsMenu\(\) \{.*?\n    \}\n\n    function GetNytPageTargets\(\)''',
...
# Compact title = title/date/byline all in one left-justified row.
# Hide Yesterday/Help = hide the native shared row, no proxy cloning.
sub_once(
    r'''    function ApplyNytTitleArrangement\(\) \{.*?\n    \}\n\n    function ApplyNytPagePreferences''',
...
# CSS: list-container owns the adjustable space; the grid itself has
# zero TI/GB row gap and the word row sizes to its actual contents.
replace_n(
    '''                grid-template-rows:
...
replace_n(
    ...
    2,
    "word-container intrinsic height"
)
...
replace_once(
    ...
    "list-container adjustable space"
)

# Replace beta.1 title/proxy CSS with compact layout + row-hide CSS.
sub_once(
    r'''            #portal-game-header\.lb-cubed-title-meta-layout \{.*?            \.lb-cubed-original-yesterday-hidden \{\n                display: none !important;\n            \}\n''',
...
''',
    "title arrangement CSS"
)

# Settings visual subgroup for the gap controls.
settings_css_marker = '''            .lb-cubed-settings-checkbox,
            .lb-cubed-settings-range,
            .lb-cubed-settings-number-row,
            .lb-cubed-settings-action-row {'''
settings_css = '''            .lb-cubed-settings-subgroup {
                margin-top: 6px;
                padding: 6px;
...
'''
replace_once(settings_css_marker, settings_css + settings_css_marker, "settings subgroup CSS")

path.write_text(text, encoding="utf-8")

changelog = Path("CHANGELOG.md")
ch = changelog.read_text(encoding="utf-8")
insert = '''## 1.12.0-beta.2
